...
void InsertAtPosition(struct DNode **head, int data, int pos) {
    ...
    if (pos == 1) { InsertAtBeginning(head, data); return; }
    struct DNode *temp = *head;
...
void DeleteByValue(struct DNode **head, int value) {
    if (!*head) { printf("List is empty. Cannot delete.\n"); return; }
    struct DNode *temp = *head;
    while (temp && temp->data != value) temp = temp->next;
    if (!temp) { printf("Element %d not found.\n", value); return; }
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 3:
                ...
                InsertAtPosition(&head, data, pos); break;
            case 4:
                printf("Enter value to delete: "); scanf("%d", &data);
                DeleteByValue(&head, data); break;
            case 5:
                DisplayForward(head); break;
            case 6:
                DisplayBackward(head); break;
            case 7:
                printf("Exiting...\n");
                while (head) {
                    struct DNode *t = head;
